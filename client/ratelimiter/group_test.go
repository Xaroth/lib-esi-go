...
func TestNewGroup(t *testing.T) {
	...
	testCases := []struct {
		...
		limitHeader   string
		expectedError error
		expectedGroup *ratelimiter.Group
	}{
		{
			name:          "valid header",
			nameHeader:    "test",
		...
		},
		{
			...
			expectedError: ratelimiter.ErrInvalidHeader,
			expectedGroup: nil,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			group, err := ratelimiter.NewGroup(tc.nameHeader, tc.limitHeader)
